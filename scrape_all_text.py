...
# Upload output files as GitHub Release assets (supports up to 2 GB per file).
# Requires the gh CLI to be installed and authenticated (gh auth login).
#
#   export GH_REPO=mutuhin/Papers-Past       # your GitHub repo
#   export GH_RELEASE_TAG=scraped-data       # release tag to attach files to

GH_REPO        = os.getenv("GH_REPO",        "mutuhin/Papers-Past")
GH_RELEASE_TAG = os.getenv("GH_RELEASE_TAG", "scraped-data")
...
_gh_release_ready = False


async def _ensure_github_release() -> None:
    global _gh_release_ready
    if _gh_release_ready or not GH_REPO:
        return
    ...
    def _create():
        subprocess.run(
            ["gh", "release", "create", GH_RELEASE_TAG,
             "--repo", GH_REPO,
             "--title", "Scraped Data",
             "--notes", "Papers Past OCR text — auto-generated by scrape_all_text.py"],
            capture_output=True,
        )

    await asyncio.to_thread(_create)
    _gh_release_ready = True


async def upload_to_github(local_path: Path) -> None:
    """Upload a file as a GitHub Release asset. No-op when GH_REPO is not set."""
    if not GH_REPO:
        return
    await _ensure_github_release()

    def _upload():
        result = subprocess.run(
            ["gh", "release", "upload", GH_RELEASE_TAG, str(local_path),
    ...
    try:
        await asyncio.to_thread(_upload)
        log.info(f"  GH  ↑ github.com/{GH_REPO}/releases/tag/{GH_RELEASE_TAG}/{local_path.name}")
    except Exception as e:
        log.warning(f"  GitHub upload failed for {local_path.name}: {e}")
